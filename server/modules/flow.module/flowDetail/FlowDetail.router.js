...
const Entity = require('./FlowDetail.model');
const { findAndCountAllOptions } = require('../../../utils/requestUtil');
const { Op } = require('sequelize');

const baseUrl = '/api/flowDetail'
...
router.get(`${baseUrl}s/all`, async (req, res)=>{
    try {
        const lists = await Entity.findAll({where: req.query, order:[['code','ASC'],['signedTime','ASC']]})
        res.send(lists)
    } catch (error) {
        res.status(500).send({ success: false, message: error.message });
    }
